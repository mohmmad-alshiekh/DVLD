...
namespace PresentationLayer.Users
{
    public partial class Add_Update_User : Form
    {
        ...
        public Add_Update_User(User user)
        {
            InitializeComponent();
            _user = user;
            _userID = user.Id;
        ...
        private void _SaveUserInformation()
        {
            ...
            _user.Person = Person.GetPersonInfoByID(Convert.ToInt32( lblPersonID.Text ));
        }
        ...
        {
            if (!User.IsPersonLinkedToUser(personSearch.PersonID))
            {
        ...
        private void Add_Update_User_Load(object sender, EventArgs e)
        {
            ...
            {
                if(_user == null)
                     _user = User.GetUserInfoByID(_userID);
        ...
        private void txtPassword_LeaveText(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (DataValidator.IsStrongPassword(txtPassword.Text))
                        errorPassword.Clear();
            }
            catch(Exception ex)
            {
